...
import { XINPLAYER_ABSENT } from "../constants/PubSubListeners";
...
class Server {
  ...
  private spawnPubSub = () => {
    this.pubsub.on(XINPLAYER_ABSENT, (playerId: string) => {
      Emitter.broadcast("Absent");
      Emitter.emit(XINPLAYER_ABSENT, playerId);
    });
  };
