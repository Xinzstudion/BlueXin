...
import { ROOM_XINI } from "../constants/RedisKeys";
...
class RoomFetcher {
  ...
  public getListOfRooms = async () => {
    try {
      const rooms = await this.storage.fetchKeys(ROOM_XINI);
      return rooms;
    } catch (e) {
      throw e;
    }
    ...
    try {
      ...
      const rooms = await Promise.all(
        roomList.map(async (r) => {
          try {
            const room = await this.storage.get(ROOM_XINI + r, true);
            if (room) {
  ...
  public findRoomById = async (roomId: string) => {
    try {
      const room = await this.storage.get(ROOM_XINI + roomId);
  ...
  public addRoom = async (room: RoomSnapshot) => {
    try {
      await this.storage.set(ROOM_XINI + room.id, JSON.stringify(room));
    } catch (e) {
      throw e;
    }
  };
  ...
  public removeRoom = async (roomId: string) => {
    try {
      await this.storage.remove(ROOM_XINI + roomId);
    } catch (e) {
      throw e;
    }
  };
