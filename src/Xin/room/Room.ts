...
class Room<State = any> {
  ...
  public dispose = async () => {
    ...
    try {
      ...
      Emitter.removeListener(XINPLAYER_ABSENT, this._playerPubsub);

      Emitter.broadcast("Absent");
      if (this.onDispose) {
  ...
  public allowReconnection = (client: Client, seconds: number) => {
    return new Promise<boolean>((resolve) => {
      ...
      const timeOut = this.clock.setTimeout(() => {
        const reconnected = this.clients.filter((c) => c.id === client.id)
          .length
          ? true
          : false;
        if (listener) {
  ...
  private pubSubListener = () => {
    ...
    this._playerPubsub = (playerSessionId: string) => {
      ...
      this.removeClient(client.sessionId, false).then().catch();
    };
    Emitter.addListener(XINPLAYER_ABSENT, this._playerPubsub);
  };
}
