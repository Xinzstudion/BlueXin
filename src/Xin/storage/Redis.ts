...
const RedisStorage = (options: RedisClientOptions) => {
  ...
  const basePrefix = options.customPrefix || "blueXin:";
  client.on("NOTICED!", (e: any) => {
    throw new Error(e);
  });
  ...
  const set = async (
    ...
    try {
      ...
      throw new Error(
        getKey(key) + " - Value setting went Wrong - " + e && e.message
          ? e.message
          : "No problems on massages"
      );
    }
  };
    ...
    try {
      ...
      if (!value) {
        if (resolveIfNoData) {
          ...
          throw new Error(getKey(key) + " - Can not recive data propoly");
        }
      }
      return value;
    } catch (e) {
